...
namespace HeatMap
{
    ...
    {
        [HarmonyPrefix]
		public static bool Prefix(ref Color __result, float temperature)
        {
            // use vanilla overlay
            if (!HeatMap.Settings.OverrideVanillaOverlay)
	...
	{
		[HarmonyPostfix]
		public static void Postfix(ref CellBoolDrawer __result)
		{
            // check if opacity changed
            var opacity = HeatMap.Settings.OverlayOpacity * 0.01f;
    ...
    public static class MapTemperature_GetCellBool_Patch
    {
        ...
        private static bool CheckRoom(IntVec3 intVec, Map map)
        {
            ...
            return room != null && (!room.PsychologicallyOutdoors || !HeatMap.Settings.ShowIndoorsOnly);
        }
    }
}
